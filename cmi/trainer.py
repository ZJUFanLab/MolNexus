...
from .config import REL2CLASS as CMI_REL2CLASS
...
from .model import *
from .results import merge_hgt_cmi_summaries_if_ready
from .scoring import CMITrainingScoring
from .training_loop import CMITrainingLoop
...
logging.basicConfig(filename=str(PROJECT_ROOT / "logs" / "training.log"), filemode="w", level=logging.DEBUG,
...
class FinalTrainer(CMITrainingLoop, CMITrainingScoring, TrainingGraphBuilder, TrainingEntityExpander):

    CKPT_DIR = "result_CMI"

    def __init__(self,
                    train_target_file, val_target_file, 
                    ppi_file, mpi_file, gpi_file, ggi_file, cgi_file,
                    cpi_pos_file,
                    cpd_features_file,
            ...
            self.train_df["label"] = self.train_df["relation"].map(CMI_REL2CLASS)
            self.val_df["label"] = self.val_df["relation"].map(CMI_REL2CLASS)

            if self.train_df["label"].isna().any():
                bad = self.train_df[self.train_df["label"].isna()]["relation"].unique()
                raise ValueError(f"Unknown CMI relation in train_df: {bad}")

            if self.val_df["label"].isna().any():
                bad = self.val_df[self.val_df["label"].isna()]["relation"].unique()
                raise ValueError(f"Unknown CMI relation in val_df: {bad}")
            ...
            self.cgi_df    = pd.read_csv(cgi_file, dtype=str)
            ...
            # Although the CPI triplets come from train/val, retain only their names in the mapping without loading their entries
