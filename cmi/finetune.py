...
def build_cpi_pretrain_rel2id(cgi_df):
    """
    Reproduce the rel2id construction used in the CPI pretraining script.
    Note: the CPI script contains only background relations + CGI subrelations + CPI, not CMI-N/CMI-D/CMI-U.
    """
...
def _remap_relation_weight(pre_w, cur_w, pre_rel2id, cur_rel2id):
    ...
    Keep the newly added CMI-N/CMI-D/CMI-U relations randomly initialized.
    """
    new_w = cur_w.clone()
    copied = []
...
def load_cpi_pretrained_for_cmi(model, ckpt_path, pre_rel2id, cur_rel2id, device):
    """
    Transfer from a CPI checkpoint to a CMI model:
    ...
    if ckpt_path is None or ckpt_path.strip() == "":
        logging.info("[TRANSFER] PRETRAIN_CKPT is empty. Train CMI from scratch.")
        return
